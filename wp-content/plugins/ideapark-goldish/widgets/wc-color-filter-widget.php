...
class Ideapark_WC_Color_Filter_Widget extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of attributes to filter products in your store.', 'ideapark-goldish' );
		$this->widget_id          = 'ip_attribute_filter';
		$this->widget_name        = __( 'Goldish Filter by Attribute', 'ideapark-goldish' );
		parent::__construct();
	}
	...
	public function form( $instance ) {
		if ( ideapark_woocommerce_on() && function_exists( 'woo_variation_swatches' ) ) {
			$this->init_settings();
			parent::form( $instance );
		} else {
			esc_html_e( 'You need to activate and configure the "Woocommerce" and "Variation Swatches for WooCommerce" plugins', 'ideapark-goldish' );
		}
	}
	...
	public function init_settings() {
		...
		$this->settings = [
			'title'      => [
				'type'  => 'text',
				'std'   => __( 'Filter by', 'ideapark-goldish' ),
				'label' => __( 'Title', 'ideapark-goldish' ),
			],
			'attribute'  => [
				'type'    => 'select',
				'std'     => $std_attribute,
				'label'   => __( 'Attribute', 'ideapark-goldish' ),
				'options' => $attribute_array,
			],
			'count'      => [
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Show product counts', 'ideapark-goldish' ),
			],
			'query_type' => [
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'ideapark-goldish' ),
				'options' => [
					'and' => __( 'AND', 'ideapark-goldish' ),
					'or'  => __( 'OR', 'ideapark-goldish' ),
				],
			],
		];
	}
	...
	public function widget( $args, $instance ) {
		...
		$found = $this->layered_nav_list( $terms, $taxonomy, $query_type, $count );
	 ...
	 */
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $with_count ) {
		// List display.
		...
		foreach ( $available_type_keys as $_type ) {
			if ( wvs_wc_product_has_attribute_type( $_type, $taxonomy ) ) {
				$type = $_type;
				break;
			}
		}
				...
				if ( $type ) {
					switch ( $type ) {
						...
						case 'image':
							...
							$swatches      = sprintf( '<span class="c-ip-attribute-filter__sw c-ip-attribute-filter__sw--image variable-item"><img class="c-ip-attribute-filter__thumb" aria-hidden="true" alt="%s" src="%s" width="%d" height="%d" /></span>', esc_attr( $term->name ), esc_url( $image[0] ), esc_attr( $image[1] ), esc_attr( $image[2] ) );
							break;
