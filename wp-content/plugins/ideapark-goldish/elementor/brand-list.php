...
class Ideapark_Elementor_Brand_List extends Widget_Base {
	...
	public function get_title() {
		return __( 'Brand List', 'ideapark-goldish' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			...
			[
				'label' => __( 'Brand list settings', 'ideapark-goldish' ),
			]
		);
		if ( ideapark_woocommerce_on() && ideapark_mod( 'product_brand_attribute' ) ) {
			...
			if ( $is_archive ) {
				$this->add_control(
					...
					[
						...
						'default' => 'logos',
						'options' => [
							'logos' => __( 'Logos', 'ideapark-goldish' ),
							'alpha' => __( 'Alphabetically', 'ideapark-goldish' ),
						]
					]
				);
			} else {
				$this->add_control(
					...
					[
						...
						'raw'   => '<a target="_blank" href="' . esc_url( admin_url( 'edit.php?post_type=product&page=product_attributes&edit=' . $attribute_id ) ) . '">' . __( 'Enable archives for brand attribute', 'ideapark-goldish' ) . '</a>',
					]
				);
			}
		} else {
			$this->add_control(
				...
				[
					'label' => '',
					'type'  => \Elementor\Controls_Manager::RAW_HTML,
					'raw'   => '<a target="_blank" href="' . esc_url( admin_url( 'customize.php?autofocus[control]=product_brand_attribute' ) ) . '">' . __( 'Set up brands to activate the widget', 'ideapark-goldish' ) . '</a>',
				]
			);
		}
