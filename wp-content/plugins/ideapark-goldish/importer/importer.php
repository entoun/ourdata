...
class Ideapark_Importer {
	...
	function admin_menu() {
		add_theme_page( __( 'Import Demo Content', 'ideapark-goldish' ), __( 'Import Demo Content', 'ideapark-goldish' ), 'manage_options', 'ideapark_themes_importer_page', [
			$this,
			'importer_page'
		] );
	}

	public function catch_error_message( $code, $msg, $file, $line ) {
		$this->session_start_error .= $msg . "\n";
	}
	...
	public function scripts( $hook ) {
		...
		if ( ! session_id() ) {
			if ( ! headers_sent() ) {
				set_error_handler( [ $this, 'catch_error_message' ] );
				$this->is_session_start = @session_start();
				restore_error_handler();
				if ( $this->is_session_start && session_status() != 2 ) {
		...
		wp_localize_script( 'ideapark-importer', 'ideapark_wp_vars_importer', [
			...
			'please_wait'  => __( 'Please wait...', 'ideapark-goldish' ),
			'are_you_sure' => __( 'Are you sure you want to import?', 'ideapark-goldish' ),
			'importing'    => __( 'Importing ...', 'ideapark-goldish' ),
			'progress'     => __( 'Progress', 'ideapark-goldish' ),
			'output_error' => __( 'Output Error', 'ideapark-goldish' ),
		] );
	}
	...
	function importer_page() {
		...
		if ( is_wp_error( $wp_filesystem->errors ) && $wp_filesystem->errors->get_error_code() == 'empty_hostname' ) {
			...
			$output .= '<h1>' . IDEAPARK_NAME . ' - ' . __( 'One-Click Import Demo Content', 'ideapark-goldish' ) . '</h1>';
		...
		foreach ( $folders as $name => $folder ) {
			if ( $folder['type'] == 'd' && $wp_filesystem->exists( $theme_title_fn = $this->importer_dir . '/' . $name . '/theme.txt' ) ) {
				...
				if ( class_exists( 'Ideapark_Fonts' ) && $wp_filesystem->exists( $fd = $this->importer_dir . '/' . $name . '/fonts/' ) ) {
				...
				foreach ( $files as $file_name => $file ) {
					if ( $file['type'] == 'f' && preg_match( '~(home-\d)\.php~', $file_name, $match ) ) {
						...
						if ( $wp_filesystem->exists( $fn = $this->importer_dir . '/' . $name . '/' . $theme_home['screenshot'] ) ) {
		...
		if ( ! empty( $themes ) ) {
			$output   .= '<p><span class="subheader">' . __( 'Select the demo site you want to import: ', 'ideapark-goldish' ) . '</span></p>';
			...
			foreach ( $themes as $name => $theme ) {
				$preview_button = '';
				if ( ! empty( $theme['url'] ) ) {
					$preview_button = '<a class="ip-demo-preview" href="' . esc_url( $theme['url'] ) . '" target="_blank">' . __( 'Preview Demo', 'ideapark-goldish' ) . '</a>';
				}
				$data = 'data-revslider="' . ( ! empty( $theme['revslider'] ) ? 'yes' : 'no' ) . '" data-fonts="' . ( ! empty( $theme['fonts'] ) ? 'yes' : 'no' ) . '"';

				if ( ! empty( $theme['screenshot'] ) ) {
					$output .= '<li class="ip-demo" ' . $data . '><label><img class="ip-screenshot" alt="' . esc_attr( $theme['title'] ) . '" src="' . esc_attr( $theme['screenshot'] ) . '" /><input class="ip-import-demo" type="radio" name="import_demo" value="' . esc_attr( $name ) . '" ' . ( $is_first ? 'checked' : '' ) . '/>' . esc_attr( $theme['title'] ) . '</label>' . $preview_button . '</li>';
				} else {
		...
		if ( ! empty( $themes ) || IDEAPARK_DEMO ) {
			$output .= '<p' . ( ( ! $is_fonts && ! $is_revslider ) ? ' class="ip-invisible"' : '' ) . '><span class="subheader">' . __( 'Select the data you want to import: ', 'ideapark-goldish' ) . '</span>' .
			           '<br />
			<label><input type="radio" name="import_option" value="all" checked/>' . __( 'All data', 'ideapark-goldish' ) . '</label><br />
			';
			if ( $is_fonts ) {
				$output .= '<label class="ip-fonts-radio"><input type="radio" name="import_option" value="fonts"/>' . __( 'Fonts', 'ideapark-goldish' ) . '</label><br />';
			}
			if ( $is_revslider ) {
				$output .= '<label class="ip-rev-slider-radio"><input type="radio" name="import_option" value="revslider"/>' . __( 'Slider Revolution', 'ideapark-goldish' ) . '</label><br />';
			}
			$output .= '</p>';
			if ( PHP_OS == "Darwin" && IDEAPARK_DEMO ) {
				$output .= '<label><input type="radio" name="import_option" value="options"/>' . __( 'Options', 'ideapark-goldish' ) . '</label><br />';
			}

			$output .= '<p><label><input type="checkbox" value="1" name="import_attachments" checked /> ' . __( 'Import attachments', 'ideapark-goldish' ) . '</label></p>';
			if ( ! $this->is_session_start ) {
				$output .= '<p class="ip-import-error">' . __( 'PHP sessions are configured incorrectly. Contact your hosting provider to fix this problem.', 'ideapark-goldish' ) . ( $this->session_start_error ? '<p><code class="ip-import-error-log">' . nl2br( trim( esc_html( $this->session_start_error ) ) ) . '</code></p>' : '' ) . '</p>';
			}
			if ( ( $code = ideapark_get_purchase_code() ) && $code !== IDEAPARK_SKIP_REGISTER ) {
				$output .= '<p class="submit">' . ( PHP_OS == "Darwin" || ! IDEAPARK_DEMO ? '<button ' . ( $this->is_session_start ? '' : ' disabled ' ) . ' class="button button-primary" id="ip-import-submit">' . __( 'Import', 'ideapark-goldish' ) . '</button> ' : '' ) . ( defined( 'IDEAPARK_DEMO' ) && IDEAPARK_DEMO ? '<button class="button button-primary" id="ip-export-submit">' . __( 'Export', 'ideapark-goldish' ) . '</button> ' : '' ) . '</p>';
				$output .= '<div class="ip-loading-progress"><div class="ip-loading-bar"><div class="ip-loading-state"></div><div class="ip-loading-info">' . __( 'Progress', 'ideapark-goldish' ) . ': 0%...</div></div><div class="ip-import-output">' . __( 'Prepare data...', 'ideapark-goldish' ) . '</div></div>';
				$output .= '<div class="ip-import-notes">';
				$output .= __( 'Important notes:', 'ideapark-goldish' ) . '<br />';
				$output .= __( 'Please note that import process will take time needed to download all attachments from demo web site.', 'ideapark-goldish' ) . '<br />';
				$output .= __( 'If you plan to use shop, please install WooCommerce before you run import.', 'ideapark-goldish' ) . '<br />';
				$output .= sprintf( wp_kses( __( 'We recommend you to <a href="%s" target="_blank">reset data</a> & clean wp-content/uploads folder before import to prevent duplicate content.', 'ideapark-goldish' ), [ 'a' => [ 'href' => [] ] ] ), esc_url( 'https://wordpress.org/plugins/wp-reset/' ) ) . '<br />';
				if ( ! ini_get( 'allow_url_fopen' ) ) {
					$output .= '<div class="error">' . esc_html__( 'Downloading remote files is prohibited in PHP (allow_url_fopen in php.ini)', 'ideapark-goldish' ) . '</div>';
				} elseif ( ! isset( $_GET['download_test'] ) ) {
					$output .= '<p>' . esc_html__( 'If you have problems importing images, then run a test to get information about the error', 'ideapark-goldish' ) . ': <a href="' . admin_url( 'themes.php?page=ideapark_themes_importer_page&download_test' ) . '">' . esc_html__( 'download test', 'ideapark-goldish' ) . '</a></p>';
				} else {
					...
					if ( is_wp_error( $result ) ) {
						$output .= '<div class="error">' . esc_html__( 'Download test failed', 'ideapark-goldish' ) . ': ' . $result->get_error_message() . '</div>';
					} else {
						$output .= '<p class="warning">' . esc_html__( 'Download test completed successfully', 'ideapark-goldish' ) . '</p>';
						ideapark_delete_file( $result['file'] );
					}
				}
				$output .= '</div>';
			} else {
				$output .= '<p class="ideapark-goldish-warning">' . esc_html__( 'Available only to registered theme', 'ideapark-goldish' ) . '</p>';
				$output .= '<p><a class="button button-primary" href="' . admin_url( 'themes.php?page=ideapark_about' ) . '">' . __( 'Register theme', 'ideapark-goldish' ) . '</a></p>';
			}
		} else {
			$output .= '<div>' . __( 'No themes data found.', 'ideapark-goldish' ) . '</div>';
			$output .= '<div>' . __( 'Try adding this code to the beginning of the file wp-config.php:', 'ideapark-goldish' ) . '</div>';
	...
	function importer() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			$this->import_response( 'error', __( 'Error: Permission denied', 'ideapark-goldish' ) );
		}
		...
		if ( ! headers_sent() ) {
			...
			$this->import_response( 'error', __( 'Error: Could not start session! Please try to turn off debug mode and error reporting', 'ideapark-goldish' ) );
		}
			...
			if ( isset( $_REQUEST['import_option'] ) ) {
				...
				if ( ! empty( $_REQUEST['import_demo'] ) ) {
					...
					if ( ! $wp_filesystem->exists( $theme_title_fn = $this->importer_dir . '/' . $this->demo_content_folder . '/' ) ) {
						$this->import_response( 'error', __( 'Error: Select the demo site you want to import', 'ideapark-goldish' ) );
					}
				}
				...
				switch ( $_REQUEST['import_option'] ) {
					...
					default:
						$this->import_response( 'error', __( 'Error: Select the data you want to import', 'ideapark-goldish' ) );
				}
		...
		switch ( $step ) {
			...
			case 'prepare':
				...
				$message = __( 'Prepared data successfully', 'ideapark-goldish' );
			...
			case 'terms':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported terms successfully', 'ideapark-goldish' );
			...
			case 'post':
				...
				if ( ! $this->importer['base']->importing() ) {
					array_shift( $this->importer['steps'] );
					$message                           = __( 'Imported post data successfully', 'ideapark-goldish' );
					$this->importer['base']->step_done += $among;
				} else {
			...
			case 'options':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported options successfully', 'ideapark-goldish' );
			...
			case 'fonts':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported fonts successfully', 'ideapark-goldish' );
			...
			case 'widget':
				...
				array_shift( $this->importer['steps'] );
				$this->importer['base']->step_done += $among;
				$message                           = __( 'Imported widgets successfully', 'ideapark-goldish' );
			...
			case 'completed':
				...
				if ( ! count( $this->importer['base']->error_msg ) ) {
					$message = '<b style="color:#444">' . __( 'Cheers! The demo data has been imported successfully! Please reload this page to finish!', 'ideapark-goldish' ) . '</b>';
				} else {
					$permalinks_settings_url = get_admin_url( null, 'options-permalink.php' );
					$message                 = '<b style="color:#444">' . __( 'Data import completed!', 'ideapark-goldish' ) . '</b>' . '<p><a class="button" href="' . $permalinks_settings_url . '">' . __( 'Re-save the site permalinks! ', 'ideapark-goldish' ) . '</a></p>' . '<div>' . implode( '', $this->importer['base']->error_msg ) . '</div>';
				}
				$code = 'completed';
			...
			default:
				$this->import_response( 'error', __( 'Error: step not found: ', 'ideapark-goldish' ) . $step );
				break;
		}
		...
		if ( $step == 'completed' ) {
			unset( $_SESSION['ideapark_importer'] );
		} else {
	...
	function import_options( $is_preliminary = false, $home = '' ) {
		global $wp_filesystem, $wpdb, $theme_home;
		...
		if ( ! $is_preliminary ) {
			...
			$theme_home       = [];
			if ( $wp_filesystem->exists( $theme_home_fn ) ) {
...
//				$wp_filesystem->put_contents( $theme_options_fn . '.txt', base64_decode( $theme_options_txt ) );
				...
				$this->import_response( 'error', __( 'Error: file not found: ', 'ideapark-goldish' ) . $theme_options_fn );
			}
		}
	...
	function import_revslider() {
		...
		if ( $this->slides ) {
			if ( class_exists( 'RevSlider' ) ) {
				...
				if ( ! $has_errors ) {
					$message = esc_html__( 'Imported Slider Revolution successfully', 'ideapark-goldish' );
				}

			} else {
				$this->importer['base']->error_msg[] = $message = __( 'The plugin Slider Revolution is not installed', 'ideapark-goldish' );
			}
		} else {
			$message = __( 'There are no slides in this demo', 'ideapark-goldish' );
		}
	...
	function exporter() {
		...
		if ( ! $wp_filesystem->is_dir( $this->export_path ) ) {
			if ( ! $wp_filesystem->mkdir( $this->export_path, 0755 ) ) {
				$this->import_response( 'error', __( 'Error: Permission denied', 'ideapark-goldish' ) . ': ' . $this->export_path );
			}
		}
		...
		$code    = 'completed';
		$message = '<b style="color:#444">' . __( 'The demo data has been exported successfully!', 'ideapark-goldish' ) . '</b>';
	...
	function import_fonts() {
		...
		if ( class_exists( 'Ideapark_Fonts' ) ) {
			...
			if ( $wp_filesystem->exists( $theme_fonts_fn ) && $wp_filesystem->is_dir( $theme_fonts_dir ) ) {
				$fonts = unserialize( base64_decode( $wp_filesystem->get_contents( $theme_fonts_fn ) ) );
	...
	function export_fonts() {
		...
		if ( ! empty( $fonts_info['fonts'] ) ) {
			...
			if ( $fonts ) {
				...
				if ( $wp_filesystem->exists( $fonts_fn ) ) {
					$wp_filesystem->delete( $fonts_fn );
				}

				$wp_filesystem->put_contents( $fonts_fn, base64_encode( serialize( $_fonts ) ) );
...
//			echo "<b>REV SLIDER</b>: " . $val_orig . ' ' . $data->settings->wp->rev_slider . '<br>';
	...
	function prepere_post_meta( $mode ) {
		global $wpdb;
		$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->postmeta} WHERE meta_key = %s", $mode ? '_elementor_data_goldish' : '_elementor_data' ) );
		foreach ( $postmeta as $meta ) {
			...
			if ( $mode ) {
				...
				delete_post_meta( $meta->post_id, '_elementor_data_goldish' );
			} else {
//				update_post_meta( $meta->post_id, '_elementor_data_goldish', '' );
//				$table = _get_meta_table( 'post' );
//				$wpdb->update( $table, [ 'meta_value' => wp_json_encode( $data ) ], [ 'post_id'  => $meta->post_id,
//				                                                                      'meta_key' => '_elementor_data_goldish'
//				] );
				update_post_meta( $meta->post_id, '_elementor_data_goldish', wp_slash( wp_json_encode( $data ) ) );
			}
		}
	...
	public function query_filter( $query ) {
		$query = str_replace( "wp_posts.post_status != 'auto-draft'", "(wp_posts.post_status != 'auto-draft' AND wp_posts.post_status != 'draft')", $query );
	...
	public function on_wp_import_post_meta( $post_meta ) {
		foreach ( $post_meta as &$meta ) {
			if ( '_elementor_data_goldish' === $meta['key'] ) {
	...
	public function on_wxr_importer_pre_process_post_meta( $post_meta ) {
		if ( '_elementor_data_goldish' === $post_meta['key'] ) {
