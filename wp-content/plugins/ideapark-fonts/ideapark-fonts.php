...
//						'url'           => $this->upload_url . $fonts_info['<css>'],
	...
	private function _add( $movefile, $version = '' ) {
		...
		if ( $movefile && empty( $movefile['error'] ) ) {
			...
			if ( ! $font_dir ) {
				...
				return new WP_Error( 'ideapark_unzip_file_error', __( 'Can`t recognize SVG font file', 'ideapark-fonts' ) );
			}
			...
			$orig_css = '';
			foreach ( $files as $file ) {
				if ( preg_match( '~\.(eot|svg|ttf|woff)$~i', $file, $match ) ) {
	...
	src: url("' . $font_urls['eot'] . '#iefix") format("embedded-opentype"),
	url("' . $font_urls['woff'] . '") format("woff"),
	url("' . $font_urls['ttf'] . '") format("truetype"),
	url("' . $font_urls['svg'] . '") format("svg");
	font-weight: normal;
	font-style: normal;
	font-display: swap;
}
