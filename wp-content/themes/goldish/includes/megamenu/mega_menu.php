	...
	public function out_custom_nav_fields( $item_id, $item, $depth, $args, $id ) {
		...
		<p class="ip-field-custom ip-field-custom--depth-0 ip-field-custom--content">
			<label for="edit-menu-item-content-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Submenu Content', 'goldish' ); ?><br>
				<select id="edit-menu-item-content-<?php echo esc_attr( $item_id ); ?>"
					...
					<option
						value="default" <?php selected( "default", $item->content ); ?>><?php esc_html_e( 'Default', 'goldish' ); ?></option>
					<option class="ip-field-custom__option-html-block"
							value="html_block" <?php selected( "html_block", $item->content ); ?>><?php esc_html_e( 'HTML block', 'goldish' ); ?></option>
					<option
						value="product_category" <?php selected( "product_category", $item->content ); ?>><?php esc_html_e( 'Product subcategories', 'goldish' ); ?></option>
					<option
						value="product_attr" <?php selected( "product_attr", $item->content ); ?>><?php esc_html_e( 'Product attribute terms', 'goldish' ); ?></option>
				</select>
			</label>
		</p>
		...
		<p class="ip-field-custom ip-field-custom--primary-0 ip-field-custom--columns">
			<label for="edit-menu-item-columns-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Submenu Columns', 'goldish' ); ?><br>
				<select id="edit-menu-item-columns-<?php echo esc_attr( $item_id ); ?>" class="edit-menu-item-columns"
					...
					<option
						value="1" <?php selected( "1", $item->columns ); ?>><?php esc_html_e( '1 column', 'goldish' ); ?></option>
					<option
						value="2" <?php selected( "2", $item->columns ); ?>><?php esc_html_e( '2 columns', 'goldish' ); ?></option>
					<option
						value="3" <?php selected( "3", $item->columns ); ?>><?php esc_html_e( '3 columns', 'goldish' ); ?></option>
					<option
						value="4" <?php selected( "4", $item->columns ); ?>><?php esc_html_e( '4 columns', 'goldish' ); ?></option>
				</select>
			</label>
		</p>
		...
		<p class="ip-field-custom ip-field-custom--primary-0 ip-field-custom--expand">
			<label for="edit-menu-item-expand-<?php echo esc_attr( $item_id ); ?>">
				...
				<?php esc_html_e( 'Expand third level', 'goldish' ); ?><br>
			</label>
		</p>

		<?php foreach (
			[
				'html_block'       => esc_html__( 'HTML Block', 'goldish' ),
				'product_category' => esc_html__( 'Parent Category', 'goldish' ),
				'product_attr'     => esc_html__( 'Attribute', 'goldish' )
			...
			<a class="ip-field-custom__loader ip-field-custom__loader--box" href=""
			   onclick="return false;">
						<?php if ( ! empty( $item->$field ) ) {
							...
							switch ( $field ) {
								case 'html_block':
									esc_html_e( 'Select HTML block', 'goldish' );
									break;
								case 'product_category':
									esc_html_e( 'Select category', 'goldish' );
									break;
								case 'product_attr':
									esc_html_e( 'Select attribute', 'goldish' );
									break;
							}
							echo ' &mdash;';
						} ?>
		...
		<p class="ip-field-custom ip-field-custom--primary-0 ip-field-custom--icon" data-type="icon"
		   data-item-id="<?php echo esc_attr( $item_id ); ?>">
			<label>
				<?php esc_html_e( 'Icon', 'goldish' ); ?><br>
				<input type="hidden" class="ip-field-custom__val"
		...
		<?php if ( ! $item->badge_text ) { ?>
			<p class="ip-field-custom ip-field-custom--primary-not-0 ip-field-custom--badge-add">
				<a onclick="return false"
				   class="ip-field-custom__badge_add button-link"><?php esc_html_e( 'Add badge', 'goldish' ); ?></a>
			</p>
		<?php } ?>
		...
		<p class="ip-field-custom ip-field-custom--left ip-field-custom--primary-not-0 ip-field-custom--badge_text <?php if ( ! $item->badge_text ) { ?>ip-field-custom--hidden<?php } ?>">
			<label for="edit-menu-item-badge_text-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Badge text', 'goldish' ); ?><br>
		...
		<p class="ip-field-custom ip-field-custom--right ip-field-custom--primary-not-0 ip-field-custom--badge_color <?php if ( ! $item->badge_text ) { ?>ip-field-custom--hidden<?php } ?>">
			<label for="edit-menu-item-badge_color-<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Badge color', 'goldish' ); ?><br>
				<span>
	...
	public function select_icon() {
		...
		if ( $item_id && $type ) {
			switch ( $type ) {
				...
				case 'html_block':
					wp_dropdown_pages(
						[
							...
							'echo'              => 1,
							'show_option_none'  => '&mdash; ' . esc_html__( 'Select HTML block', 'goldish' ) . ' &mdash;',
							'option_none_value' => '0',
							'selected'          => $value,
				...
				case 'product_category' :
					$list = [
						''       => '&mdash; ' . esc_html__( 'Select parent category', 'goldish' ) . ' &mdash;',
						'-shop-' => esc_html__( 'Shop', 'goldish' ),
					];
				...
				case 'product_attr' :
					$list = [
						'' => '&mdash; ' . esc_html__( 'Select attribute', 'goldish' ) . ' &mdash;',
					];
...
//		wp_enqueue_media();
		...
		wp_localize_script( $this->_token . '-megamenu', 'ideapark_wp_vars_mega_menu', [
			...
			'placeholderIcon' => esc_html__( "Select icon", 'goldish' ),
		] );
	}
