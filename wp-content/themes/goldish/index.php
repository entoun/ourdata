...
<div
	class="c-blog l-section <?php ideapark_class( ideapark_mod( 'post_layout' ) == 'grid', 'l-section--container-wide', 'l-section--container' ); ?> l-section--top-margin-60 l-section--bottom-margin <?php if ( $with_sidebar ) { ?> l-section--with-sidebar<?php } ?>">
	<div
		...
		<div class="c-blog-wrap <?php ideapark_class( ideapark_mod( 'post_layout' ) == 'grid', 'c-blog-wrap--grid', 'c-blog-wrap--list' ); ?><?php ideapark_class( $with_sidebar, ' c-blog-wrap--sidebar', ' c-blog-wrap--no-sidebar' ); ?>">
			<?php if ( have_posts() ): ?>
				...
				<div class="c-blog__nothing">
					<div
						class="c-blog__nothing-text"><?php esc_html_e( 'We could not find any results for your search. You can give it another try through the search form below:', 'goldish' ); ?>
					</div>
