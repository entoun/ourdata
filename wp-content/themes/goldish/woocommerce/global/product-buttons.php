...
if ( ideapark_mod( 'shop_modal' ) || ideapark_mod( 'wishlist_page' ) && ideapark_mod( 'wishlist_grid_button' ) ) { ?>
	<div class="c-product-grid__thumb-button-list <?php ideapark_class( ideapark_mod( 'product_grid_layout_mobile' ) == '2-per-row-mobile' && ideapark_mod( 'hide_add_to_cart_mobile_2_per_row' ) || ideapark_mod( 'product_grid_layout_mobile' ) == 'compact-mobile' && ideapark_mod( 'hide_add_to_cart_mobile_compact' ) , 'c-product-grid__thumb-button-list--hide-mobile' ); ?>">
		<?php if ( ideapark_mod( 'shop_modal' ) ) { ?>
			<button class="h-cb c-product-grid__thumb-button c-product-grid__thumb-button--quickview js-grid-zoom" type="button" data-lang="<?php echo esc_attr( apply_filters( 'wpml_current_language', null ) ); ?>" data-product-id="<?php echo esc_attr( $product->get_id() ); ?>">
