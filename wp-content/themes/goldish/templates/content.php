...
if ( is_page() ) {
	...
	<?php if ( ! ideapark_mod( 'post_hide_author' ) ) { ?>
		<li class="c-page__meta-item">
			<span class="c-page__meta-by"><?php esc_html_e( 'by', 'goldish' ); ?></span>
			<?php the_author_link(); ?>
		</li>
	<?php } ?>
	...
	<?php if ( ! ideapark_mod( 'post_hide_comment' ) ) { ?>
		...
		<?php if ( $comments_count->total_comments > 0 ) { ?>
			<li class="c-page__meta-item">
				<?php esc_html_e( 'Comments:', 'goldish' ); ?><?php echo esc_html( $comments_count->total_comments ); ?>
			</li>
		<?php } ?>
	<?php } ?>
...
<article
	...
	<div
		...
		<div class="c-post__inner">
			...
			<div
				class="c-post__content h-clearfix entry-content <?php ideapark_class( ideapark_mod( 'sidebar_post' ), 'entry-content--sidebar', 'entry-content--fullwidth' ); ?>">
				<?php the_content( '<span class="c-post__more-button">' . esc_html__( 'Continue Reading', 'goldish' ) . '</span>' ); ?>
			</div>
			...
			<?php if ( ! $is_page && ( ! ideapark_mod( 'post_hide_share' ) || ! ideapark_mod( 'post_hide_tags' ) ) ) { ?>
				...
				<?php if ( ! ideapark_mod( 'post_hide_tags' ) && has_tag() ) { ?>
					<div class="c-post__tags">
						<?php the_tags( '', '<i class="ip-romb c-post__tags-bullet"></i>' ); ?>
					</div>
				<?php } ?>
