...
define( 'IDEAPARK_NAME', 'Goldish' );
define( 'IDEAPARK_SLUG', 'goldish' );
...
define( 'IDEAPARK_MANUAL', 'https://parkofideas.com/goldish/manual/' );
define( 'IDEAPARK_SUPPORT', 'https://parkofideas.ticksy.com/' );
define( 'IDEAPARK_CHANGELOG', 'https://themeforest.net/item/goldish-jewelry-store-woocommerce-theme/33978103#item-description__goldish-wordpress-theme-changelog' );
define( 'IDEAPARK_VERSION', '2.18' );
...
if ( ! function_exists( 'ideapark_setup' ) ) {
	...
	function ideapark_setup() {
		...
		load_theme_textdomain( 'goldish', IDEAPARK_DIR . '/languages' );
		...
		register_nav_menus( [
			'primary' => esc_html__( 'Primary (Desktop)', 'goldish' ),
			'mobile'  => esc_html__( 'Primary (Mobile)', 'goldish' ),
		] );
	}
}
...
if ( ! function_exists( 'ideapark_get_required_plugins' ) ) {
	function ideapark_get_required_plugins() {
		...
		$plugins = [
			[
				'name'     => esc_html__( 'Goldish Core', 'goldish' ),
				'slug'     => 'ideapark-goldish',
				'source'   => IDEAPARK_DIR . '/plugins/ideapark-goldish.zip',
				'required' => true,
				'version'  => IDEAPARK_VERSION,
			],

			[
				'name'     => esc_html__( 'Font Icons Loader', 'goldish' ),
				'slug'     => 'ideapark-fonts',
			...
			[
				'name'     => esc_html__( 'WooCommerce', 'goldish' ),
				'slug'     => 'woocommerce',
				'required' => true
			],

			[
				'name'     => esc_html__( 'Meta Box', 'goldish' ),
				'slug'     => 'meta-box',
				'required' => true,
			],

			[
				'name'     => esc_html__( 'Elementor', 'goldish' ),
				'slug'     => 'elementor',
				'required' => true
			],

			[
				'name'     => esc_html__( 'Contact Form 7', 'goldish' ),
				'slug'     => 'contact-form-7',
				'required' => false,
			],

			[
				'name'     => esc_html__( 'Variation Swatches for WooCommerce', 'goldish' ),
				'slug'     => 'woo-variation-swatches',
				'required' => false,
			],

			[
				'name'           => esc_html__( 'Revolution Slider', 'goldish' ),
				'slug'           => 'revslider',
				'type'           => 'api',
				"source"         => 'revslider/revslider.php',
				'version'        => '6.5.21',
				'tooltip'        => esc_html__( 'This plugin is not used in the demo. Install it only if you are going to create custom slides, because it slows down the site and negatively affects PageSpeed', 'goldish' ),
				'required'       => false,
				'notice_disable' => true,
			],
		];
...
if ( ! function_exists( 'ideapark_widgets_init' ) ) {
	function ideapark_widgets_init() {
		...
		register_sidebar( [
			'name'          => esc_html__( 'Post List', 'goldish' ),
			'id'            => 'post-sidebar',
		...
		if ( ideapark_woocommerce_on() ) {
			if ( ideapark_mod( 'single_sidebar' ) ) {
				register_sidebar( [
					'name'          => esc_html__( 'Product List', 'goldish' ),
					'id'            => 'shop-sidebar',
					'before_widget' => '<aside id="%1$s" class="widget %2$s">',
					'after_widget'  => '</aside>',
					'before_title'  => '<div class="widget-title">',
				...
				register_sidebar( [
					'name'          => esc_html__( 'Product List (Desktop)', 'goldish' ),
					'id'            => 'shop-sidebar',
					'before_widget' => '<aside id="%1$s" class="widget %2$s">',
					'after_widget'  => '</aside>',
					'before_title'  => '<div class="widget-title">',
				...
				register_sidebar( [
					'name'          => esc_html__( 'Product List (Mobile)', 'goldish' ),
					'id'            => 'filter-sidebar',
					'before_widget' => '<aside id="%1$s" class="widget %2$s">',
					'after_widget'  => '</aside>',
			...
			if ( $query->have_posts() ) {
				...
				<div class="c-header-search__view-all">
					<button class="c-button c-button--outline js-ajax-search-all"
							type="button"><?php echo esc_html__( 'View all results', 'goldish' ); ?></button>
				</div>
			<?php } else { ?>
				<div
					class="c-header-search__no-results"><?php echo esc_html__( 'No results found', 'goldish' ); ?></div>
			<?php }
...
if ( ! function_exists( 'ideapark_category' ) ) {
	function ideapark_category( $separator, $cat = null, $a_calss = '' ) {
		...
		foreach ( $cat as $category ) {
			$catetories[] = '<a ' . ( $a_calss ? 'class="' . esc_attr( $a_calss ) . '"' : '' ) . ' href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . sprintf( esc_attr__( "View all posts in %s", 'goldish' ), $category->name ) . '" ' . '>' . esc_html( $category->name ) . '</a>';
		}
		...
		<article id="div-comment-<?php comment_ID(); ?>"
				...
				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID, $args ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( esc_html_x( '%1$s at %2$s', '1: date, 2: time', 'goldish' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
				</div>

				<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'goldish' ); ?></p>
				<?php endif; ?>
			</header>
			...
			<div class="buttons">
				<?php comment_reply_link( array_merge( $args, [
					'reply_text' => esc_html__( 'Reply', 'goldish' ),
					'add_below'  => 'div-comment',
					'depth'      => $depth,
					'max_depth'  => $args['max_depth']
				] ) ); ?>

				<?php edit_comment_link( esc_html__( 'Edit', 'goldish' ) ); ?>
			</div>
	...
	function ideapark_search_form( $form ) {
		...
		$form = '<form role="search" method="get" class="js-search-form-entry" action="' . esc_url( home_url( '/' ) ) . '">
				...
				<label class="c-search-form__label">
					<span class="screen-reader-text">' . esc_html_x( 'Search for:', 'label', 'goldish' ) . '</span>
					<input class="c-form__input c-form__input--full c-search-form__input" type="search" placeholder="' . esc_attr_x( 'Search &hellip;', 'placeholder', 'goldish' ) . '" value="' . get_search_query() . '" name="s" />' .
		        '</label>
...
<form role="search" class="js-search-form" method="get" action="' . esc_url( home_url( '/' ) ) . '">
	<div class="c-header-search__input-block">
		<input class="js-ajax-search-input c-header-search__input' . ( ! ideapark_mod( 'ajax_search' ) ? ' no-ajax' : '' ) . '" autocomplete="off" type="text" name="s" placeholder="' . esc_attr__( 'Start typing...', 'goldish' ) . '" value="' . esc_attr( ideapark_mod( 'ajax_search' ) ? '' : get_search_query() ) . '" />
		<button class="js-search-clear h-cb c-header-search__clear' . ( ! ideapark_mod( 'ajax_search' ) ? ' no-ajax' : '' ) . '" type="button"><i class="ip-close-small c-header-search__clear-svg"></i><span class="c-header-search__clear-text">' . esc_html__( 'Clear', 'goldish' ) . '</span></button>
...
<form role="search" class="js-search-form" method="get" action="' . esc_url( home_url( '/' ) ) . '">
	<div class="c-header__search-form">
		<input class="js-ajax-search-input c-header__search-input' . ( ! ideapark_mod( 'ajax_search' ) ? ' no-ajax' : '' ) . '" autocomplete="off" type="text" name="s" placeholder="' . esc_attr__( 'Start typing...', 'goldish' ) . '" value="' . esc_attr( ideapark_mod( 'ajax_search' ) ? '' : get_search_query() ) . '" />
		<button class="js-search-clear h-cb c-header__search-clear' . ( ! ideapark_mod( 'ajax_search' ) ? ' no-ajax' : '' ) . '" type="button"><i class="ip-close-small c-header__search-clear-svg"></i><span class="c-header__search-clear-text">' . esc_html__( 'Clear', 'goldish' ) . '</span></button>
...
if ( ! function_exists( 'ideapark_get_account_link' ) ) {
	function ideapark_get_account_link( $prefix = '' ) {
		$link_title = '<i class="ip-user h-hide-mobile"><!-- --></i><i class="ip-m-auth h-hide-desktop"><!-- --></i>';

		return ideapark_wrap( $link_title, '<a class="c-header__button-link c-header__button-link--account" title="' . esc_attr( is_user_logged_in() ? esc_html__( 'Account', 'goldish' ) : esc_html__( 'Login', 'goldish' ) ) . '" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '" rel="nofollow">', '</a>' );
	}
}
...
if ( ! function_exists( 'ideapark_localize_vars' ) ) {
	function ideapark_localize_vars() {
		...
		$return = [
			...
			'viewMore'          => esc_html__( 'View More', 'goldish' ),
			...
			'hideButtons'       => ideapark_mod( 'product_grid_layout_mobile' ) == '2-per-row-mobile' && ideapark_mod( 'hide_add_to_cart_mobile_2_per_row' ) || ideapark_mod( 'product_grid_layout_mobile' ) == 'compact-mobile' && ideapark_mod( 'hide_add_to_cart_mobile_compact' ),
			'ajaxAddToCart'     => ideapark_mod( 'product_page_ajax_add_to_cart' ) && 'yes' === get_option( 'woocommerce_enable_ajax_add_to_cart' ),
		];
		...
		if ( ideapark_woocommerce_on() && ideapark_mod( 'wishlist_page' ) && function_exists( 'ideapark_wishlist' ) ) {
			$return = array_merge( $return, [
				...
				'wishlistTitleAdd'    => esc_html__( 'Add to Wishlist', 'goldish' ),
				'wishlistTitleRemove' => esc_html__( 'Remove from Wishlist', 'goldish' )
			] );
		}
...
if ( ! function_exists( 'ideapark_custom_meta_boxes' ) ) {
	function ideapark_custom_meta_boxes( $meta_boxes ) {
		...
		$fields = [
			[
				'name'              => __( 'HTML block (top)', 'goldish' ),
				'label_description' => '<a target="_blank" href="' . esc_url( admin_url( 'edit.php?post_type=html_block' ) ) . '">' . esc_html__( 'Manage html blocks', 'goldish' ) . '</a>',
				'id'                => 'html_block_top',
				'type'              => 'post',
				'post_type'         => 'html_block',
				'field_type'        => 'select_advanced',
				'placeholder'       => __( 'Select a HTML block', 'goldish' ),
				'query_args'        => [
					'post_status'    => [ 'publish' ],
					'posts_per_page' => - 1,
				],
			...
			[
				'name'              => __( 'HTML block (bottom)', 'goldish' ),
				'label_description' => '<a target="_blank" href="' . esc_url( admin_url( 'edit.php?post_type=html_block' ) ) . '">' . esc_html__( 'Manage html blocks', 'goldish' ) . '</a>',
			...
			[
				'name'      => esc_html__( 'Show HTML blocks only on the first page', 'goldish' ),
			...
			[
				'name'      => esc_html__( 'Show top block above the shop content', 'goldish' ),
				'id'        => 'top_block_above',
				'type'      => 'switch',
				'style'     => 'rounded',
				'on_label'  => 'Yes',
...
if ( ! function_exists( 'ideapark_breadcrumb_list' ) ) {
	function ideapark_breadcrumb_list() {
		...
		$breadcrumb_home_text             = __( 'Home', 'goldish' );
		...
		if ( is_front_page() && is_home() ) {
			...
			if ( ! empty( $permalink_structure ) && get_post_type() == 'post' ) {
				...
				if ( $is_page_for_posts || ! $breadcrumb_display_home ) {
					$array_list[] = [
						...
						'title'    => $is_page_for_posts ? get_the_title( get_option( 'page_for_posts' ) ) : __( 'Posts', 'goldish' ),
						'location' => 'is_category',
					];
				}
			...
			if ( $taxonomy == 'category' ) {
				$array_list[] = [
					'link'     => $is_page_for_posts ? get_permalink( get_option( 'page_for_posts' ) ) : get_home_url(),
					'title'    => $is_page_for_posts ? get_the_title( get_option( 'page_for_posts' ) ) : __( 'Blog', 'goldish' ),
					'location' => 'is_category',
				];
			} else {
				$array_list[] = [
			...
			$array_list[] = [
				'link'     => '#',
				'title'    => __( 'Page not found', 'goldish' ),
				'location' => 'is_404',
			];
